...
export function useLocalGenerate() {
  ...
  useEffect(() => {
    ...
    if (savedDir) {
      ...
      localGenerateApi.preview(savedDir)
  ...
  const pickInputDir = useCallback(async () => {
    ...
    try {
      ...
      if (!res.data.cancelled && res.data.path) {
        ...
        setLoadingPreview(true);
        try {
          const previewRes = await localGenerateApi.preview(dir);
  ...
  const rescan = useCallback(async () => {
    ...
    setError(null);
    try {
      const previewRes = await localGenerateApi.preview(inputDir);
      _applyPreview(previewRes.data as LocalPreviewOut);
    } catch (e: unknown) {
      const err = e as { response?: { data?: { detail?: string } } };
      setError(err?.response?.data?.detail ?? "Failed to scan directory");
  ...
  const executeBatch = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await workflowsApi.executeLocalBatch({
        input_dir: inputDir,
